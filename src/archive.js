...
export async function archiveSessions(opts) {
  ...
  if (finalize) {
    if (!existsSync(pendingDir)) return { command: 'archive', toolVersion: '0.1.0', summary: { pending: 0, removed: 0 }, items: [], exitCode: 0 };
    const entries = readdirSync(pendingDir);
    if (!dryRun) rmSync(pendingDir, { recursive: true, force: true });
    return { command: 'archive', toolVersion: '0.1.0', summary: { pending: entries.length, removed: dryRun ? 0 : entries.length }, items: [{ id: '.archive-pending', status: dryRun ? 'pending' : 'removed', reason: 'finalize' }], exitCode: 0 };
  }

  // 阶段1: migrate（真实执行）
  const result = await migrate({ srcRoot, targetRoot: archiveRoot, direction, dryRun: false });
  if (result.summary.failed > 0) {
    // 有失败：中止，源不动（协议保证）
    return { command: 'archive', toolVersion: '0.1.0', summary: { ...result.summary, pending: 0 }, items: result.items, exitCode: 1, note: '阶段1有失败，源未动（两阶段协议）' };
  }
  if (dryRun) {
    return { command: 'archive', toolVersion: '0.1.0', summary: { ...result.summary, pending: result.summary.migrated + result.summary.copied }, items: result.items, exitCode: 0 };
  }
  ...
  return { command: 'archive', toolVersion: '0.1.0', summary: { ...result.summary, pending }, items: result.items, exitCode: 0, note: pending > 0 ? '暂存完成：运行 archive --finalize <源根> 清空暂存' : '无可暂存项' };
}
